...
{
int64_t	homepi,         /* value of pi calculated by current task */
...
for (i = 0; i < ROUNDS; i++) {
   ...
   rc = MPI_Reduce(&homepi, &pisum, 1, MPI_LONG_LONG, MPI_SUM,
   ...
   if (taskid == MASTER) {
      ...
      printf("   After %8d throws, average value of pi = %10.8ld\n",
              (DARTS * (i + 1)),avepi);
   }    
} 
if (taskid == MASTER) 
